...
switching_time_options=60.*[60, 75, 100, 1000];

for iexp=1:number_of_replicates_per_input_class
    
    
    % Randomly choose the maximum IPTGext value to be used in the experiment
    IPTGext_max= experimental_IPTGext_values(iexp+1); %experimental_IPTGext_values(randi([1,5],1));
    IPTGext_min= experimental_IPTGext_values(iexp); %experimental_IPTGext_values(randi([1,5],1));
...
%  
    ...
    % switching time
    switching_time=switching_time_options(iexp);%switching_time_options(randi([1,14],1)); %randi([60*60,(t_d/3)],1);                        % 250 minutes in seconds
    ...
    exps.t_s{iexp}      = (0:5*60:t_d); %(IPTGext_min:switching_time:exps.t_f{iexp});                                   % sampling time is 5 minutes (5X60=300 seconds)
